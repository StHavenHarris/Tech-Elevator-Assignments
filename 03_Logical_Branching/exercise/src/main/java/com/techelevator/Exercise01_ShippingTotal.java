...
public class Exercise01_ShippingTotal {
    ...
    public double calculateShippingTotal(int weightPounds) {
        ...
        if (weightPounds <= MAX_WEIGHT_POUNDS){
            ...
            return weightPounds * OVER_40_LB_RATE - 10;
        }
        return calculateShippingTotal(10);
    }
    ...
    public double calculateShippingTotal(int weightPounds, boolean hasDiscount) {
        ...
        if (weightPounds <= MAX_WEIGHT_POUNDS){
            shipCost = UP_TO_40_LB_RATE * weightPounds;
        } else  {
            shipCost = ((weightPounds - MAX_WEIGHT_POUNDS) * OVER_40_LB_RATE)  + (MAX_WEIGHT_POUNDS * UP_TO_40_LB_RATE);
        }
        if (hasDiscount){
            shipCost = shipCost * 0.9;
        }
        return shipCost;
    }
    ...
    public double calculateShippingTotal(int weightPounds, double discountPercentage) {
        ...
        if (weightPounds <= MAX_WEIGHT_POUNDS){
            shipCost = UP_TO_40_LB_RATE * weightPounds;
            double discAmount = shipCost * discountPercentage;
            double totalShipCost = shipCost - discAmount;
    ...
    public static void main(String[] args){
        Exercise01_ShippingTotal object = new Exercise01_ShippingTotal();
        System.out.print(object.calculateShippingTotal(45, 0.2) + " 19.0");
    }
}
