...
public class Exercise05_AcceptPackage {
    ...
    public boolean acceptPackage(int weightPounds, int lengthInches, int widthInches, int heightInches, boolean isSurchargePaid) {
        int multiInches = lengthInches * widthInches * heightInches;
        boolean isWeightAtLimit = weightPounds <= MAX_WEIGHT_POUNDS;
        boolean isMaxCIAtLimit = multiInches <= MAX_CUBIC_INCHES;
        boolean isDimenAtLimit = (lengthInches <= MAX_DIMENSION_INCHES) && (widthInches <= MAX_DIMENSION_INCHES) && (heightInches <= MAX_DIMENSION_INCHES);

        if (isWeightAtLimit && isMaxCIAtLimit && isDimenAtLimit){
        return true;
        } else if (isWeightAtLimit && isMaxCIAtLimit && !isDimenAtLimit && isSurchargePaid){
            return true;
        } else {
            return false;
        }
