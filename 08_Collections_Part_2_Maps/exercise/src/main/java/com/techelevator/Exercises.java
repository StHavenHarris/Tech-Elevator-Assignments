...
public class Exercises {
	...
	public Map<String, Integer> robPeterToPayPaul(Map<String, Integer> peterPaul) {
		...
		if (peterMon > 0 && paulMon < 1000) {
			if (peterMon % 2 != 0) {
				peterMon = peterMon / 2;
				paulMon = peterMon + paulMon;
				peterMon++;
			} else if (peterMon > 0 && paulMon < 1000) {
				peterMon = peterMon / 2;
				paulMon = peterMon + paulMon;
			} else {
				return peterPaul;
		...
		peterPaul.put("Peter", peterMon);
		peterPaul.put("Paul", paulMon);
	...
	public Map<String, Integer> peterPaulPartnership(Map<String, Integer> peterPaul) {
		...
		if (peterMon >= 5000 && paulMon >= 10000) {
			int peterPaulPartnershipPeter = peterMon / 4;
			peterMon = peterMon - peterPaulPartnershipPeter;
			int peterPaulPartnershipPaul = paulMon / 4;
			paulMon = paulMon - peterPaulPartnershipPaul;
	...
	public Map<String, String> beginningAndEnding(String[] words) {
		...
		for (String strings : words) {
			...
			keyValues.put(strings.substring(0, 1), strings.substring(strings.length() - 1));
		}
	...
	public Map<String, Integer> last2Revisited(String[] words) {
		Map<String, Integer> strAndCount = new HashMap<>();
		...
		for (String x : words) {
			if (words.length == 0) {
				...
				int count = 0;
				String twoCount = x.substring(x.length() - 2);

				for (int i = 0; i < x.length() - 2; i++) {

					if (x.substring(i, i + 2).equals(twoCount)) {

						count++;
					}
				}
				strAndCount.put(x, count);
			}
		}
		return strAndCount;
	}
