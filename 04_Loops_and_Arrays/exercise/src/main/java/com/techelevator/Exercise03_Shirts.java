...
public class Exercise03_Shirts {
    ...
    public char[] buildBulkOrder(int numberOfShirts) {
        char[] shirtsInBlk = new char[numberOfShirts];

        for (int i = 0; i < shirtsInBlk.length; i++) {
            if (i % 3 == 0) {
                shirtsInBlk[i] = SMALL_TSHIRT;
            }
            if (i % 3 == 1){
                shirtsInBlk[i] = MEDIUM_TSHIRT;
            }
            if (i % 3 == 2){
                shirtsInBlk[i] = LARGE_TSHIRT;
            }
        }
        return shirtsInBlk;
    }
